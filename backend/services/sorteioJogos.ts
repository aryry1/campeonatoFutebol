import { buscaTimes } from "../repositores/times";

export async function geraSorteio() {
    try {

        const times = await buscaTimes()

        if (times.length != 8) {
            throw new Error("Quantidade de times insuficientes para gerar os jogos. Cadastre mais times no campeonato")
        }
